...
//        private static final long serialVersionUID=1;
        ...
        void setKey(String n_key){
            String chKey = n_key.toUpperCase().trim();
            if(chKey.equals(null) || chKey.equals("")){
                throw new IllegalArgumentException("Invalid key");
            }
            key = chKey;
    ...
    private Object findRecursive(String fKey,TreeNode curNd){
        ...
        else if(fKey.equals(curNd.getKey())){
    ...
    public DSAQueue inOrder(){
        DSAQueue inOrderList = new Queue();
        if(root == null){
    ...
    private DSAQueue inorderRecurse(TreeNode curNd, DSAQueue inorderList){
        if(curNd != null){
            inorderRecurse(curNd.getLeft(),inorderList);
            inorderList.enQueue(curNd.getKey());
